 ...
 */
	...
	$.jstree.defaults.core = {
		 ...
		 *
		 * It is possible to pass in a standard jQuery-like AJAX es and jstree will automatically determine if the response is JSON or HTML and use that to populate the tree.
	...
	$.jstree.core.prototype = {
		...
		redraw_node : function (node, deep, is_callback, force_render) {
			...
			if((obj.icon && obj.icon !== true) || obj.icon === false) {
				if(obj.icon === false) {
					...
					node.childNodes[1].childNodes[0].className += ' ' + obj.icon + ' jstree-themeicon-customVO';
				}
				else {
					node.childNodes[1].childNodes[0].style.backgroundImage = 'url('+obj.icon+')';
					node.childNodes[1].childNodes[0].style.backgroundPosition = 'center center';
					node.childNodes[1].childNodes[0].style.backgroundSize = 'auto';
					node.childNodes[1].childNodes[0].className += ' jstree-themeicon-customVO';
				}
			}
		...
		check : function (chk, obj, par, pos, more) {
			...
			if(chc === false || ($.isFunction(chc) && chc.call(this, chk, obj, par, pos, more) === false) || (chc && chc[chk] === false)) {
				this._data.core.last_error = { 'error' : 'check', 'plugin' : 'core', 'id' : 'core_03', 'reason' : 'User es for core.check_callback prevents function: ' + chk, 'data' : JSON.stringify({ 'chk' : chk, 'pos' : pos, 'obj' : obj && obj.id ? obj.id : false, 'par' : par && par.id ? par.id : false }) };
				return false;
			}
			return true;
		},
		...
		set_icon : function (obj, icon) {
			...
			if(icon === false) {
				...
				dom.removeClass('jstree-themeicon-customVO ' + old).css("background","").removeAttr("rel");
				if(old === false) { this.show_icon(obj); }
			}
			else if(icon.indexOf("/") === -1 && icon.indexOf(".") === -1) {
				dom.removeClass(old).css("background","");
				dom.addClass(icon + ' jstree-themeicon-customVO').attr("rel",icon);
				if(old === false) { this.show_icon(obj); }
			}
			else {
				dom.removeClass(old).css("background","");
				dom.addClass('jstree-themeicon-customVO').css("background", "url('" + icon + "') center center no-repeat").attr("rel",icon);
				if(old === false) { this.show_icon(obj); }
			}
			return true;
		},
 ...
 */
	 ...
	 *
	 * It is possible to set this to a standard jQuery-like AJAX es.
 ...
 */
	...
	$.jstree.defaults.search = {
		/**
		 * a jQuery-like AJAX es, which jstree uses if a server should be queried for results.
		 *
