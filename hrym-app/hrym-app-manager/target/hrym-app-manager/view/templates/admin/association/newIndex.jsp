            ...
            <div class="ibox ">
                ...
                <div class="ibox-content">
                    <div class="input-group col-md-3"
                         style="float:left;margin-right:0px;margin-top:10px;positon:relative">
                    ...
                    <div class="input-group col-md-3"
                         style="float:left;margin-left:30px;margin-bottom: 15px;margin-top:10px;mpositon:relative">
                        <select class="input-group col-md-3" id="columnName" name="columnName"
                                style="width: 260px;height: 31px">
                    ...
                    <div style="float:left;margin-left: 180px;margin-top:10px;positon:relative">
                        <div class="myBtn-content">
                            <button class="btn btn-success" type="button"
                                    onclick="window.location='${pageContext.request.contextPath}/view/templates/admin/association/form.jsp'"
...
<script>
    $(document).ready(function () {
        ...
        $("#table_list").bootstrapTable({
            ...
            //获取数据的Servlet地址
            url: "${pageContext.request.contextPath}/admin/newFindAllArticle",
            //表格显示条纹
            striped: false,
            //启动分页
            pagination: true,
            ...
            columns: [{
                ...
                formatter: function (value, row, index) {
                    if (value == "0") {
                        return '<button class="btn btn-success btn-xs" type="button" onclick="publish (\'' + row.articleId + '\')"> 未发表</button>';
                    }
                    else if (value == "1") {
                        return '<button class="btn btn-primary btn-xs" type="button"  onclick="down(\'' + row.articleId + '\')"> 已发表</button>';
                    }
                }
            }, {
                title: "专栏名称",
                ...
                width: 110,
                formatter: function (value, row, index) {
                    var operateHtml = '<button class="btn btn-primary btn-xs" type="button" onclick="editArticle(\'' + row.articleId + '\')"><i class="fa fa-edit"></i>&nbsp;修改</button> &nbsp;';
                    operateHtml = operateHtml + '<button class="btn btn-danger btn-xs" type="button" onclick="del(\'' + row.articleId + '\')"><i class="fa fa-remove"></i>&nbsp;删除</button> &nbsp;';
                    return operateHtml;
                }
            }]
        });
    ...
    function editArticle(id) {
        layer.open({
            ...
            maxmin: true,
            area: ['95%', '95%'],
            content: 'http://www.everglamming.com/hrym-web/dist/index.html?id=' + id,
            end: function (index) {
    ...
    function publish(id) {
        ...
        layer.confirm('此功能不能扯回，确定发表吗?', {icon: 3, title: '提示'}, function (index) {
            $.ajax({
                ...
                dataType: "json",
                contentType: "application/json",
                url: "${pageContext.request.contextPath}/admin/newUpdateStatusArticle/",
                data: {"articleId": id},
    ...
    function down(id) {
        ...
        layer.confirm('此功能不能扯回，确定下架吗?', {icon: 3, title: '提示'}, function (index) {
            $.ajax({
                type: "GET",
                dataType: "json",
                contentType: "application/json",
                url: "${pageContext.request.contextPath}/admin/newUpdateArticleStatus/",
                data: {"articleId": id},
                success: function (msg) {
                    layer.msg(msg.message, {time: 2000}, function () {
                        $('#table_list').bootstrapTable("refresh");
    ...
    function del(id) {
        layer.confirm('确定删除吗?', {icon: 3, title: '提示'}, function (index) {
            $.ajax({
                ...
                dataType: "json",
                contentType: "application/json",
                url: "${pageContext.request.contextPath}/admin/newDeleteArticle/",
                data: {"Id": id},
                success: function (msg) {
                    layer.msg(msg.message, {time: 2000}, function () {
                        $('#table_list').bootstrapTable("refresh");
    ...
    function eventquery() {
        ...
        $('#table_list').bootstrapTable('refresh', {url: '${pageContext.request.contextPath}/admin/newFindArticleByArticleTitle?articleTitle=' + encodeURI(encodeURI(articleTitle, "UTF-8"))});
    }
    //专栏名称搜索
    function columnName() {
        var columnName = $('#columnName').val();
        $('#table_list').bootstrapTable('refresh', {url: '${pageContext.request.contextPath}/admin/newFindArticleByArticleTitle?columnName=' + encodeURI(encodeURI(columnName, "UTF-8"))});
    }
    //重置
    function reset() {
        var columnName = $('#columnName').val('');
        var articleTitle = $('#articleTitle').val('');
        $('#table_list').bootstrapTable('refresh', {url: '${pageContext.request.contextPath}/admin/newFindArticleByArticleTitle'});
    }
