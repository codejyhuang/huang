                ...
                <div class="ibox-content">
                    <div id="toolbar" style="float:left;margin-top:10px;positon:relative;">
                        <div class="myBtn-content">
                            <button type="button" id="a" class="btn btn-primary" value="10000"style="display: none" disabled="true" onclick="chose(value)">功课</button>
                        </div>
                    </div>
...
//                formatter: function (value, row, index) {
//                    if(value == 1)
//                        return '<button class="btn btn-primary btn-xs"  type="button" onclick="updateIsNotEnable(\'' + row.contentId  + '\')">未启用 </button>';
//                    else if(value == 2)
//                        return '<button class="btn btn-sucess btn-xs"  type="button"  onclick="updateIsEnable(\'' + row.contentId + '\')"> 已启用</button>';
//                }
            },{
                title: "操作",
                field: "empty",
    ...
    function publish(id){
        layer.confirm('确定发布吗?', {icon: 3, title:'提示'}, function(index){
            ...
            $.ajax({
                ...
                dataType: "json",
                contentType: "application/json",
                url: "${pageContext.request.contextPath}/admin/publishTaskItem/",
                data:{"itemId":id,"userId":userId,"typeId":typeId},
                success: function(msg){
                    layer.msg(msg.message, {time: 2000},function(){
                        $('#table_list').bootstrapTable("refresh");
    ...
    function updateIsNotEnable(itemContentId) {
        ...
        layer.confirm('确定启用吗?', {icon: 3, title: '提示'}, function (index) {
            $.ajax({
                ...
                dataType: "json",
                contentType: "application/json",
                url: "${pageContext.request.contextPath}/admin/updateMBIsEnabledById",
                data: {"isEnabled":2,"itemContentId":itemContentId,"typeId":typeId},
                success: function (msg) {
                    layer.msg(msg.message, {time: 2000}, function () {
                        $('#table_list').bootstrapTable("refresh");
                        layer.close(index);
    ...
    function updateIsEnable(itemContentId) {
        ...
        layer.confirm('确定不启用吗?', {icon: 3, title: '提示'}, function (index) {
            $.ajax({
                ...
                contentType: "application/json",
                url: "${pageContext.request.contextPath}/admin/updateMBIsEnabledById",
                data: {"isEnabled":1,"itemContentId":itemContentId,"typeId":typeId},
                success: function (msg) {
                    layer.msg(msg.message, {time: 2000}, function () {
                        $('#table_list').bootstrapTable("refresh");
                        layer.close(index);
