                    ...
                    <div class="ibox-content">
                        <div  class="form-inline form-table">
                            <div class="form-group">
                                <input type="text" class="form-control search-name" name="keywords" id="keywords"  placeholder="问题模糊搜索" />
                                ...
                                <input type="text" class="form-control search-name" id="startTimeis"name="startTimeis"
                                       placeholder="YYYY-MM-DD hh:mm:ss" value="${qo.startTimeis}" onclick="laydate({istime: true, format: 'YYYY-MM-DD hh:mm:ss'})"style="width: 180px; margin-left: 16px" />&nbsp;-
                                <input type="text" class="form-control search-name" id="endTimeis" name="endTimeis"
                                       placeholder="YYYY-MM-DD hh:mm:ss" value="${qo.endTimeis}" onclick="laydate({istime: true, format: 'YYYY-MM-DD hh:mm:ss'})" style="width: 180px; margin-left: 16px"  />
    ...
    <script>
        $(document).ready(function () {
			...
			$("#table_list").bootstrapTable({
			    ...
			    //获取数据的Servlet地址
			    url: "${pageContext.request.contextPath}/admin/selectAllQuestion",
			    //表格显示条纹
			    striped: false,
			    //启动分页
			    pagination: true,
               ...
               // height: tableHeight(),
			    ...
			    columns: [{
			        ...
			        field: "empty",
                    formatter: function (value, row, index) {
                    	var operateHtml = '<button class="btn btn-primary btn-xs" type="button" data-target="#myModal" data-toggle="modal" onclick="setValue(\''+row.id+'\',\''+row.questionName+'\',\''+row.uuid+'\')" ><i class="fa fa-edit"></i>&nbsp;回答</button> &nbsp;';
                        operateHtml = '<button class="btn btn-primary btn-xs" type="button" onclick="viewAnswer(\''+row.id+'\')"><i class="fa fa-edit"></i>&nbsp;查看</button> &nbsp;' + operateHtml ;
                        return operateHtml;
                    }
			    }]
			});
        ...
        function setValue(questionId,questionName,uuid) {
            $("#questionId").val(questionId);
            $("#questionName").html(questionName);
            $("#questionUserId").val(uuid);
        }

        function viewAnswer(id) {
            window.location.href='${pageContext.request.contextPath}/admin/queryAnswer?questionId='+id;
        }
        ...
        function timeBanner() {
            ...
            $('#table_list').bootstrapTable('refresh', { url: '${pageContext.request.contextPath}/admin/selectAllQuestion?startTimeis=' + startTimeis +'&endTimeis='+endTimeis +'&keywords='+encodeURI(encodeURI(keywords,"UTF-8"))});
        ...
        function answerInsert() {
            ...
            $.ajax({
                ...
                processData:false,
                contentType:false,
                url: "${pageContext.request.contextPath}/admin/answerInsert",
                data:form,
            ...
            <div class="modal-content modal-style">
                ...
                <div class="modal-body">
                    <form class="form-horizontal" role="form" id="model_from" action="">
                        <input type="hidden" id="questionId" name="questionId" value="" placeholder="ID"/>
                        <input type="hidden" id="uuid" name="uuid" value="-1" />
                        <input type="hidden" id="questionUserId" name="questionUserId"  />
                        <div class="form-group">
                            <label class="col-sm-2 control-label">问题:</label>
                            <div class="col-sm-10 div1" aria-required="true" id="radioType">
                                <label class="col-sm-2 control-label" id="questionName"></label>
                            </div>
                        </div>
                        <div class="form-group">
                            <label class="col-sm-2 control-label">选择头像:</label>
                            <div class="col-sm-10">
                                <input class="form-control lW" id="headImage" name="headImage"  type="text" placeholder="xxxx.jpg" readonly>
                            </div>
                        </div>
                        <div class="form-group">
                            <div class="col-sm-3">
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-10 ">
                                <input class="form-control lW" id="userName" name="userName" type="text" placeholder="xxx法师">
                            </div>
                        </div>
                        <div class="form-group">
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-primary save" data-dismiss="modal" id="btn" onclick="answerInsert()">提交
                    </button>
                </div>
            </div><!-- /.modal-content -->
        </div><!-- /.modal -->
