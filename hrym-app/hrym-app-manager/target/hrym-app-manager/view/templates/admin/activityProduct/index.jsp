                ...
                <div class="ibox float-e-margins" id="itemContent">
                    ...
                    <div class="ibox-content">
                        <form class="form-horizontal m-t" id="bannerForm" method="post" action="">
                            ...
                            <div class="form-group">
                                <label class="col-sm-3 control-label">礼品价格：</label>
                                <div class="col-sm-8">
                            ...
                            <div class="form-group">
                                <label class="col-sm-3 control-label">礼品数量：</label>
                                <div class="col-sm-8">
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-sm-8 ">
                                    <select class="form-control lW" name="activityId" id="activityId"
                                            onchange="return activityList(value)" onclick="activityList(value)">
                                    </select>
                                </div>
                            </div>
                        </form>
...
<script>
    $(document).ready(function () {
        ...
        $("#table_list").bootstrapTable({
            ...
            //获取数据的Servlet地址
            url: "${pageContext.request.contextPath}/admin/findAllProduct",
            //表格显示条纹
            striped: false,
            //启动分页
            pagination: true,
            ...
            columns: [{
                ...
                formatter: function (value, row, index) {
                    var operateHtml = '<button class="btn btn-primary btn-xs" data-toggle="modal" data-backdrop="static" data-target="#myModal" type="button" onclick="updateCom(\'' + row.id + '\')"><i class="fa fa-edit"></i>&nbsp;修改</button> &nbsp;';
                    operateHtml= operateHtml +'<button class="btn btn-primary btn-xs" type="button" onclick="delActivity(\'' + row.id + '\')"><i class="fa fa-delicious"></i>&nbsp;删除</button> &nbsp;';
                    return operateHtml;
                }
            }]
        });
    ...
    function updateCom(id) {
        ...
        $.ajax({
            ...
            dataType: "json",
            contentType: "application/json",
            url: "${pageContext.request.contextPath}/admin/findProductById/",
            data: {"Id": id},
...
//                }
                Apply(1);
                $("#id").val(msg.rows.id);
                $("#productName").val(msg.rows.productName);
                $("#productPrice").val(msg.rows.productPrice);
                $("#totalAmount").val(msg.rows.totalAmount);
                $("#activityId").val(msg.rows.activityId);
                //下拉框默认选中事件
                $("#activityId").append("<option name='activityId'  value=" + msg.rows.activityId + ">" + msg.rows.activityName + "</option>");
            }
        });
    }
    ...
    $(document).on("click", ".save", function () {
        if (id.value.length > 0) {
            url = "${pageContext.request.contextPath}/admin/updateProduct"
        } else {
            url = "${pageContext.request.contextPath}/admin/insertProduct"
        }
    ...
    function delActivity(id) {
        layer.confirm('确定删除吗?', {icon: 3, title: '提示'}, function (index) {
            ...
            $.ajax({
                ...
                dataType: "json",
                contentType: "application/json",
                url: "${pageContext.request.contextPath}/admin/deleteProdeuct/",
                data: {"id": id},
                success: function (msg) {
                    layer.msg(msg.message, {time: 2000}, function () {
                        $('#table_list').bootstrapTable("refresh");
    ...
    function userList(id) {
        ...
        layer.open({
            ...
            shade: false,
            area: ['1030px', '550px'],
            content: '${pageContext.request.contextPath}/admin/findAllUserlist?uuid=' + id,
            end: function (index) {
                $('#table_list').bootstrapTable("refresh");
            }
        });
   ...
   function Apply( value) {
        ...
        $.ajax({
            ...
            dataType: "json",
            contentType: "application/json",
            url: "${pageContext.request.contextPath}/admin/findInforMation",
            data: {"itemId": value},
            success: function (msg) {
                var columnNameList = msg.rows;
                for (var i = 0; i < columnNameList.length; i++) {
                    $("#activityId").append("<option name='activityId'  value=" + columnNameList[i].activityId + ">" + columnNameList[i].activityTitle + "</option>");
                }
            }
        });
