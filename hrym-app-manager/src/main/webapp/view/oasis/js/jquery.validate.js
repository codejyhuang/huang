	...
	validate: function( options ) {
		...
		if ( validator.settings.onsubmit ) {
			...
			this.submit( function( event ) {
				...
				// prevent submit for invalid forms or customVO submit handlers
...
$.extend( $.validator, {
	...
	prototype: {
		...
		init: function() {
			...
			function delegate( event ) {
				...
				if ( settings[ eventType ] ) {
					...
					if(this.next('.oas-select').size() > 0 && this.is( settings.ignore )){
						settings[ eventType ].call( validator, this[ 0 ], event );
					}else if(!this.is( settings.ignore )){
						settings[ eventType ].call( validator, this[ 0 ], event );
					}
		...
		elements: function() {
			...
			.filter( function() {
				...
				if( $(this).is('select') && $(this).next('.oas-select').size() > 0 ){
					return true;
				}else if(!$(this).is(':hidden') || $(this).data("forceValidate")){
					return true;
				}
			});
		...
		// return the customVO message for the given element and validation method
		...
		// return the customVO message for the given element name and validation method
	...
	methods: {
		...
		email: function( value, element ) {
			...
			// Or use customVO methods to implement your own email validation
