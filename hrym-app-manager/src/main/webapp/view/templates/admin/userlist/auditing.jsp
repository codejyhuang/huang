    ...
    <script>
        $(document).ready(function () {
			...
			$("#table_list").bootstrapTable({
			    ...
			    //获取数据的Servlet地址
			    url: "${pageContext.request.contextPath}/admin/certification",
			    //表格显示条纹
			    striped: false,
			    //启动分页
			    pagination: true,
               ...
               // height: tableHeight(),
			    ...
			    columns: [{
			        ...
			        title: "身份证正面照",
			        field: "identityCardUrl1",
                    formatter:function (value) {
                        return '<img  src='+value+' width=100 height=50  >';
                    }
			    },{
			        title: "身份证反面照",
			        field: "identityCardUrl2",
			        ...
			        field: "empty",
                    formatter: function (value, row, index) {
                    	var operateHtml = '<button class="btn btn-primary btn-xs" type="button" onclick="pass(\''+row.uuid+'\')">&nbsp;通过</button> &nbsp;';
                    	operateHtml = operateHtml + '<button class="btn btn-danger btn-xs" type="button" onclick="refuse(\''+row.uuid+'\')">&nbsp;拒绝</button> &nbsp;';
                        return operateHtml;
                    }
			    }]
			});
        ...
        function pass(id){
        	layer.confirm('确定审核通过吗?', {icon: 3, title:'提示'}, function(index){
        		$.ajax({
                    ...
                    dataType: "json",
                    contentType: "application/json",
                    url:"${pageContext.request.contextPath}/admin/updateCertification/",
                    data:{"uuid":id,"identityAuthState":"1"},
        ...
        function refuse(id){
            layer.confirm('确定拒绝审核吗?', {icon: 3, title:'提示'}, function(index){
                $.ajax({
                    type: "GET",
                    dataType: "json",
                    contentType: "application/json",
